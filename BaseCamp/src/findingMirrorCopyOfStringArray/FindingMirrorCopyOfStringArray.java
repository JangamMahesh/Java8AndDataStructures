package findingMirrorCopyOfStringArray;
...
public class FindingMirrorCopyOfStringArray {
		...
		for (int i = 0; i < mirrorStringArray.length; i++) {
			...
			if (i >= array.length) {
				for (int j = array.length - 1; j >= 0; j--) {
					mirrorStringArray[i++] = getReverseOfString(array[j]);
				}
			} else {
	...
	private static String getReverseOfString(String string) {
